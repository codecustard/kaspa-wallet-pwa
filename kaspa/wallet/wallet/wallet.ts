...
class Wallet extends EventTargetImpl {
	...
	static networkTypes: Object = {
		hoosat: { port: 16110, network: 'hoosat', name : 'mainnet' },
	...
	constructor(privKey: string, seedPhrase: string, networkOptions: NetworkOptions, options: WalletOptions = {}) {
		super();
		this.logger = CreateLogger('HoosatWallet');
	...
	*/
	 ...
	 * @param obj.amount Amount to send in sompis (100000000 (1e8) sompis in 1 HTN)
	...
	*/
	...
	async composeTxAndNetworkFeeInfo(txParamsArg: TxSend): Promise < TxInfo >{
		...
		this.logger.info(`tx ... amount: ${HTN(txParamsArg.amount)} user fee: ${HTN(txParamsArg.fee)} max data fee: ${HTN(txParamsArg.networkFeeMax||0)}`)
		...
		this.logger.verbose(`tx ... need data fee: ${HTN(dataFee)} total needed: ${HTN(amountRequested+dataFee)}`)
		this.logger.verbose(`tx ... available: ${HTN(amountAvailable)} in ${data.utxos.length} UTXOs`)

		if(networkFeeMax && dataFee > networkFeeMax) {
			throw new Error(`Fee max is ${networkFeeMax} but the minimum fee required for this transaction is ${HTN(dataFee)} HTN`);
		}
		...
		if(calculateNetworkFee){
			do {
				...
				this.logger.verbose(`tx ... need data fee: ${HTN(dataFee)} for ${data.utxos.length} UTXOs`);
			...
			if(networkFeeMax && txParams.fee > networkFeeMax)
				throw new Error(`Maximum network fee exceeded; need: ${HTN(dataFee)} HTN maximum is: ${HTN(networkFeeMax)} HTN`);

		}else if(dataFee > priorityFee){
			throw new Error(`Minimum fee required for this transaction is ${HTN(dataFee)} HTN`);
	...
	async buildTransaction(txParamsArg: TxSend, debug = false): Promise < BuildTxResult > {
		...
		this.logger.info(`tx ... required data fee: ${HTN(dataFee)} (${utxos.length} UTXOs)`);// (${HTN(txParamsArg.fee)}+${HTN(dataFee)})`);
		//this.logger.verbose(`tx ... final fee: ${HTN(dataFee+txParamsArg.fee)} (${HTN(txParamsArg.fee)}+${HTN(dataFee)})`);
		this.logger.info(`tx ... resulting total: ${HTN(totalAmount)}`);
